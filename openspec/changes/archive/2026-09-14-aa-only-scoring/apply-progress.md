...
- **TDD:** F1 espejo RED→GREEN; F2 7 RED→GREEN (+triangulación por variantes); F3 stash-RED→GREEN; F4 5 RED→GREEN (+matriz equal/different/old/fail-soft); F5 docs-only.
    - **S3e (export filtrado):** `buildExportPayload` scope-conditional: `filtered` proyecta `rankedActive` con las 10 columnas del DOM (sin Lifecycle, sin filas reference/non-active; JSON `models`/`ranked` alineados); `full-catalog` intacto. Tests reescritos al contrato (RED: header 6-col + `Ref-Model` presente) → GREEN 34/34; nota compartida iff N>0 (N=0 sin nota, N=1 con nota); probe DOM/export 11/11.
